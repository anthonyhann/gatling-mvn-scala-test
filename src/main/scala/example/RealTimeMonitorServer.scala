...
object RealTimeMonitorServer {
  ...
  private class StatsApiHandler extends HttpHandler {
    ...
    private def generateStatsJson(): String = {
      ...
      val entries = testStats.toList
      for (i <- 0 until entries.size) {
        val (testName, stats) = entries(i)
        jsonBuilder.append(s""""$testName": {""")
        
        val statsEntries = stats.toList
        for (j <- 0 until statsEntries.size) {
          val (key, value) = statsEntries(j)
          ...
          jsonBuilder.append(s""""$key": $valueStr""")
          if (j < statsEntries.size - 1) {
            jsonBuilder.append(", ")
          }
        }
  ...
  private class PerformanceMetricsApiHandler extends HttpHandler {
    ...
    private def generatePerformanceMetricsJson(): String = {
      ...
      for (i <- 0 until entries.size) {
        ...
        for (j <- 0 until limitedMetrics.size) {
